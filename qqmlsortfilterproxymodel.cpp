...
/*!
    ...
    \brief Filters and sorts data coming from a source \l
   {http://doc.qt.io/qt-5/qabstractitemmodel.html} {QAbstractItemModel}.

    The SortFilterProxyModel type provides support for filtering and sorting
   data coming from a source model. \sa FilterContainer, SorterContainer
*/
...
#endif
{
  ...
  connect(this, &QAbstractItemModel::dataChanged, this,
          &QQmlSortFilterProxyModel::onDataChanged);
...
/*!
    ...
    Delay the execution of filters, sorters and proxyRoles until the next event
   loop. This can be used as an optimization when multiple filters, sorters or
   proxyRoles are changed in a single event loop. They will be executed once in
   a single batch at the next event loop instead of being executed in multiple
   sequential batches.

    By default, the SortFilterProxyModel is not delayed, unless the \c
   SFPM_DELAYED environment variable is defined at compile time.
*/
bool QQmlSortFilterProxyModel::delayed() const { return m_delayed; }
...
/*!
    ...
    \sa {http://doc.qt.io/qt-5/qsortfilterproxymodel.html#sortRole-prop}
   {sortRole}, roleForName
*/
...
/*!
    ...
    This property holds the list of filters for this proxy model. To be included
   in the model, a row of the source model has to be accepted by all the top
   level filters of this list.
...
/*!
    ...
    This property holds the list of sorters for this proxy model. The rows of
   the source model are sorted by the sorters of this list, in their order of
   insertion.
...
/*!
    ...
    This property holds the list of proxy roles for this proxy model. Each proxy
   role adds a new custom role to the model.
...
QVariant QQmlSortFilterProxyModel::data(const QModelIndex &index,
                                        int role) const {
  return sourceData(mapToSource(index), role);
}

QHash<int, QByteArray> QQmlSortFilterProxyModel::roleNames() const {
  return m_roleNames.isEmpty() && sourceModel() ? sourceModel()->roleNames()
                                                : m_roleNames;
}
...
/*!
    ...
    Return the item at \a row in the proxy model as a map of all its roles. This
   allows the item data to be read (not modified) from JavaScript.
*/
...
/*!
    ...
    Return the data for the given \a roleName of the item at \a row in the proxy
   model. This allows the role data to be read (not modified) from JavaScript.
...
/*!
    ...
    Returns the source model index corresponding to the given \a proxyIndex from
   the SortFilterProxyModel.
*/
QModelIndex
QQmlSortFilterProxyModel::mapToSource(const QModelIndex &proxyIndex) const {
...
/*!
    ...
    Returns the source model row corresponding to the given \a proxyRow from the
   SortFilterProxyModel. Returns -1 if there is no corresponding row.
*/
...
/*!
    \qmlmethod QModelIndex SortFilterProxyModel::mapFromSource(QModelIndex
   sourceIndex)

    Returns the model index in the SortFilterProxyModel given the \a sourceIndex
   from the source model.
*/
QModelIndex
QQmlSortFilterProxyModel::mapFromSource(const QModelIndex &sourceIndex) const {
...
/*!
    ...
    Returns the row in the SortFilterProxyModel given the \a sourceRow from the
   source model. Returns -1 if there is no corresponding row.
*/
...
bool QQmlSortFilterProxyModel::filterAcceptsRow(
  ...
  bool valueAccepted =
      ...
      (m_filterValue == sourceModel()->data(sourceIndex, filterRole()));
  bool baseAcceptsRow =
      valueAccepted &&
      QSortFilterProxyModel::filterAcceptsRow(source_row, source_parent);
  baseAcceptsRow =
      baseAcceptsRow &&
      std::all_of(m_filters.begin(), m_filters.end(), [=](Filter *filter) {
...
bool QQmlSortFilterProxyModel::lessThan(const QModelIndex &source_left,
  ...
  if (m_completed) {
    ...
    std::stable_sort(
        sortedSorters.begin(), sortedSorters.end(),
        [](Sorter *a, Sorter *b) { return a->priority() > b->priority(); });
...
void QQmlSortFilterProxyModel::setSourceModel(QAbstractItemModel *sourceModel) {
  if (sourceModel &&
      sourceModel->roleNames()
          .isEmpty()) { // workaround for when a model has no roles and roles
...
void QQmlSortFilterProxyModel::onDataChanged(const QModelIndex &topLeft,
...
void QQmlSortFilterProxyModel::queueInvalidateProxyRoles() {
  ...
  if (m_delayed) {
    if (!m_invalidateProxyRolesQueued) {
      ...
      QMetaObject::invokeMethod(this, "invalidateProxyRoles",
                                Qt::QueuedConnection);
    }
  } else {
...
void QQmlSortFilterProxyModel::onFilterAppended(Filter *filter) {
  connect(filter, &Filter::invalidated, this,
          &QQmlSortFilterProxyModel::queueInvalidateFilter);
  queueInvalidateFilter();
}
...
void QQmlSortFilterProxyModel::onFiltersCleared() { queueInvalidateFilter(); }

void QQmlSortFilterProxyModel::onSorterAppended(Sorter *sorter) {
  connect(sorter, &Sorter::invalidated, this,
          &QQmlSortFilterProxyModel::queueInvalidate);
  queueInvalidate();
}
...
void QQmlSortFilterProxyModel::onSortersCleared() { queueInvalidate(); }

void QQmlSortFilterProxyModel::onProxyRoleAppended(ProxyRole *proxyRole) {
  beginResetModel();
  connect(proxyRole, &ProxyRole::invalidated, this,
          &QQmlSortFilterProxyModel::queueInvalidateProxyRoles);
  connect(proxyRole, &ProxyRole::namesAboutToBeChanged, this,
          &QQmlSortFilterProxyModel::beginResetModel);
  connect(proxyRole, &ProxyRole::namesChanged, this,
          &QQmlSortFilterProxyModel::endResetModel);
  endResetModel();
}
...
void registerQQmlSortFilterProxyModelTypes() {
  qmlRegisterType<QQmlSortFilterProxyModel>("SortFilterProxyModel", 0, 2,
